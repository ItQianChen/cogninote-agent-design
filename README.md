...
- 桌面交付：支持构建 Windows 桌面程序和 NSIS 安装包，以及 macOS Apple Silicon `.app` / `.dmg` 独立打包链路；`0.1.1` 定位为可安装测试分发版，CI 未配置证书时产出 unsigned 测试包，配置证书后产出签名/公证分发包。
...
也可以在 GitHub Actions 手动触发 `Desktop Windows` workflow 构建。workflow 会在 Windows runner 上安装 JDK 25、Node 和 Rust；未配置证书时上传 `CogniNote-0.1.1-windows-x64-unsigned-*` 测试 artifacts，配置 `WINDOWS_CERTIFICATE_PFX_BASE64` 和 `WINDOWS_CERTIFICATE_PASSWORD` 后上传 `signed` artifacts。
...
当前 `0.1.1` 仍把 API Key 明文保存到本机 SQLite，适合内部测试和小范围安装验证，不建议作为大范围公开生产发布。公开发布前应改为 Windows 本地加密或凭据管理，并补充桌面会话令牌保护。
