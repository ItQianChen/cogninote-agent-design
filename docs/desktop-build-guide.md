...
```powershell
.\scripts\update-release-version.ps1 0.1.75
```
...
```powershell
.\scripts\update-release-version.ps1 0.1.75 -WhatIf
```
...
cogniNote-agent-front/src-tauri/target/release/bundle/nsis/CogniNote_0.1.75_x64-setup.exe
```
...
cogniNote-agent-front/src-tauri/target/release/bundle/dmg/CogniNote_0.1.75_aarch64.dmg
```
...
```text
CogniNote-0.1.75-windows-x64-unsigned-portable
CogniNote-0.1.75-windows-x64-unsigned-installer
CogniNote-0.1.75-windows-x64-signed-portable
CogniNote-0.1.75-windows-x64-signed-installer
```
...
```text
CogniNote-0.1.75-macos-arm64-unsigned-app
CogniNote-0.1.75-macos-arm64-unsigned-dmg
CogniNote-0.1.75-macos-arm64-signed-app
CogniNote-0.1.75-macos-arm64-signed-dmg
CogniNote-0.1.75-macos-notarization-logs
```
...
```text
CogniNote-0.1.75-windows-x64-signed-installer.exe
CogniNote-0.1.75-windows-x64-unsigned-installer.exe
```

macOS updater 指向 `.app.tar.gz`：

```text
CogniNote-0.1.75-macos-arm64-signed.app.tar.gz
CogniNote-0.1.75-macos-arm64-unsigned.app.tar.gz
```
...
release_tag = v0.1.75-test.1
```

`release_tag` 留空时，unsigned 构建默认发布到 `v0.1.75-test.1`，signed 构建默认发布到 `v0.1.75`。unsigned Release 会标记为 pre-release。Windows 和 macOS 可以分别运行 workflow，并使用同一个 `release_tag`，后运行的平台会把自己的资产追加到同一个 Release 中。

Release 上传的是真实安装文件，不是 Actions artifact 外层 zip。普通用户优先下载 signed 资产；unsigned 资产只给开发者做技术测试：

```text
CogniNote-0.1.75-windows-x64-unsigned-installer.exe
CogniNote-0.1.75-windows-x64-unsigned-portable.zip
CogniNote-0.1.75-macos-arm64-signed.dmg
CogniNote-0.1.75-macos-arm64-signed.app.zip
CogniNote-0.1.75-macos-arm64-signed.app.tar.gz
```
...
```bash
open ./cogniNote-agent-front/src-tauri/target/release/bundle/dmg/CogniNote_0.1.75_aarch64.dmg
```
...
如果 signed DMG 仍提示“已损坏，无法打开”，优先检查 workflow 的 `CogniNote-0.1.75-macos-notarization-logs`、`spctl` 输出，以及 `desktop-backend.log` 中的实际 app 路径。重点确认 `CogniNote.app/Contents/Resources/backend/CogniNoteBackend.app` 已用同一 Developer ID 证书签名，且发布用 DMG 是由已 staple 的 `CogniNote.app` 重新生成的。
...
`CogniNoteBackend.exe` 是后端 app-image 的一部分，负责启动 Spring Boot 服务。它不是桌面应用入口。请运行 `cogninote-agent.exe` 或安装 `CogniNote_0.1.75_x64-setup.exe`。
