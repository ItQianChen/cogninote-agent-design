...
cogniNote-agent-front/src-tauri/target/release/bundle/nsis/CogniNote_0.1.1_x64-setup.exe
```
...
cogniNote-agent-front/src-tauri/target/release/bundle/dmg/CogniNote_0.1.1_aarch64.dmg
```
...
```text
CogniNote-0.1.1-windows-x64-unsigned-portable
CogniNote-0.1.1-windows-x64-unsigned-installer
CogniNote-0.1.1-windows-x64-signed-portable
CogniNote-0.1.1-windows-x64-signed-installer
```
...
```text
CogniNote-0.1.1-macos-arm64-unsigned-app
CogniNote-0.1.1-macos-arm64-unsigned-dmg
CogniNote-0.1.1-macos-arm64-signed-app
CogniNote-0.1.1-macos-arm64-signed-dmg
CogniNote-0.1.1-macos-notarization-logs
```
...
```bash
open ./cogniNote-agent-front/src-tauri/target/release/bundle/dmg/CogniNote_0.1.1_aarch64.dmg
```
...
`CogniNoteBackend.exe` 是后端 app-image 的一部分，负责启动 Spring Boot 服务。它不是桌面应用入口。请运行 `cogninote-agent.exe` 或安装 `CogniNote_0.1.1_x64-setup.exe`。
...
如果本地未签名 `.app` 被 Gatekeeper 拦截，可在系统设置中允许运行，仅用于开发验证。若 GitHub Actions 分发产物仍提示“已损坏，无法打开”，通常说明公证或 staple 没成功，先检查 `CogniNote-0.1.1-macos-notarization-logs` artifact 和 workflow 中的 `spctl` 输出。
