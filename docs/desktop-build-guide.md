...
```powershell
.\scripts\update-release-version.ps1 0.1.31 -WhatIf
```
...
```text
CogniNote-0.1.31-windows-x64-unsigned-portable
CogniNote-0.1.31-windows-x64-unsigned-installer
CogniNote-0.1.31-windows-x64-signed-portable
CogniNote-0.1.31-windows-x64-signed-installer
```
...
```text
CogniNote-0.1.31-macos-arm64-unsigned-app
CogniNote-0.1.31-macos-arm64-unsigned-dmg
CogniNote-0.1.31-macos-arm64-signed-app
CogniNote-0.1.31-macos-arm64-signed-dmg
CogniNote-0.1.31-macos-notarization-logs
```
...
release_tag = v0.1.31-test.1
```

`release_tag` 留空时，unsigned 构建默认发布到 `v0.1.31-test.1`，signed 构建默认发布到 `v0.1.31`。unsigned Release 会标记为 pre-release。Windows 和 macOS 可以分别运行 workflow，并使用同一个 `release_tag`，后运行的平台会把自己的资产追加到同一个 Release 中。

Release 上传的是真实安装文件，不是 Actions artifact 外层 zip。普通用户优先下载 signed 资产；unsigned 资产只给开发者做技术测试：

```text
CogniNote-0.1.31-windows-x64-unsigned-installer.exe
CogniNote-0.1.31-windows-x64-unsigned-portable.zip
CogniNote-0.1.31-macos-arm64-signed.dmg
CogniNote-0.1.31-macos-arm64-signed.app.zip
```
...
```bash
open ./cogniNote-agent-front/src-tauri/target/release/bundle/dmg/CogniNote_0.1.31_aarch64.dmg
```
...
`CogniNoteBackend.exe` 是后端 app-image 的一部分，负责启动 Spring Boot 服务。它不是桌面应用入口。请运行 `cogninote-agent.exe` 或安装 `CogniNote_0.1.31_x64-setup.exe`。
