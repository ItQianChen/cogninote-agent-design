...
- 自动更新通过 `desktop-updater-stable` 和 `desktop-updater-preview` 两个固定 GitHub Release 提供 `latest.json`；设置页默认 stable，preview 需要用户显式切换。
...
- Tauri Rust commands 封装 updater 检查和安装，支持 stable/preview 两个 GitHub Release 静态 manifest endpoint。
- 设置中心新增“应用更新”，显示当前版本、通道、可用版本、release notes 和下载/安装进度；启动后自动检查一次，发现更新再提示用户确认。
- CI 发布真实 Release 资产后更新 `desktop-updater-stable` 或 `desktop-updater-preview/latest.json`；Windows updater 使用最终 NSIS installer，macOS updater 使用 `.app.tar.gz`。
