...
- 读取本地 Markdown、Word DOCX / DOC 文档和文本型 PDF；无文本层 PDF 可诊断为需 OCR，并可在配置视觉识别模型后重新解析
- 读取本地 HTML / HTM 文档
- 建立本地混合检索索引
- 通过用户配置的大模型进行问答
- 回答时提供来源引用和原文溯源
...
- 文本型 PDF 解析；无文本层 PDF 诊断为需 OCR，并支持可选视觉模型 OCR 重新解析
- Lucene 本地混合检索
- 外部 Embedding API
...
本地数据目录
  ├─ SQLite 元数据、Chunk 文本、维护队列与历史、会话和消息
  ├─ Lucene 索引
  ├─ 配置文件
  └─ 日志文件
```
...
- `.doc`
- `.html` / `.htm`
- `.pdf`，优先解析文本层；无文本层 PDF 会记录为 `OCR_REQUIRED`，配置视觉模型 OCR 后可重新解析为可搜索文本
...
- `SpringAiChatRuntime.stream(...)` 支持本轮 tools 和 toolContext。工具集合为空时不调用 `.tools(...)`；联网搜索开启时挂载 `WebSearchTools.searchWeb`，并通过 `ToolContext` 传入请求级设置快照和 `ToolExecutionCollector`。
...
当前 Embedding 配置还保存 `embedding_requests_per_minute`、`embedding_tokens_per_minute` 和 `embedding_batch_size`。这些字段是模型调用节流参数，不改变已写入向量的语义；单独修改它们不要求重建 Lucene 索引。
...
向量索引重建提示只在 active Embedding 的索引相关签名变化时出现：Provider、Base URL、模型 ID 或向量维度变化才需要提醒用户重建 Lucene 向量索引。保存未启用向量配置、修改配置名称、修改 API Key、修改 RPM/TPM/Batch，或保存时这些字段没有变化，都不能弹出重建索引提示。

模型 API Key 和联网搜索 Exa API Key 当前仍以开发态明文保存到 SQLite；本地加密或 Windows 凭据管理放到安全加固阶段，不能在最终交付版本继续明文保存。
...
ChatAgentRouter 按 useKnowledgeBase 选择 Agent
  ↓
WebSearchToolPolicy 按 useWebSearch 决定是否为本轮挂载 WebSearchTools
  ├─ useWebSearch=false/null：不读取联网设置，不挂工具
  └─ useWebSearch=true：要求全局启用、provider=EXA 且 API Key 已配置
  ↓
useKnowledgeBase=false
  └─ GENERAL_CHAT：普通对话 prompt + CogninoteMemoryAdvisor + 可选 WebSearchTools
      ↓
      调用 active CHAT 模型
      ↓
      如果模型调用 searchWeb，网页来源通过 SSE tool 事件增量推送
      ↓
      流式返回答案
      ↓
...
useKnowledgeBase=true
  └─ KNOWLEDGE_BASE：RAG prompt + CogninoteMemoryAdvisor + 可选 WebSearchTools
      ↓
...
知识库模式回答必须包含本地引用来源，避免变成不可验证的聊天工具。普通对话模式不挂 RAG 时，`retrievalMode=null` 且 SSE `meta.sources=[]`；如果本轮同时开启联网，网页来源会在工具执行后通过 `tool` 事件进入同一条 assistant 消息。联网搜索结果不写入本地知识库、Lucene 索引或知识图谱，只作为本轮回答来源保存。
...
- 资料总览页展示知识库健康概览，按目录标记 `HEALTHY`、`WARNING`、`ERROR`、`DISABLED` 或 `EMPTY`
- 停用目录是用户主动排除检索范围的状态，不参与全库问题数量、问题弹窗或 WARNING/ERROR 升级
- “查看问题”先弹出存在问题的目录清单，再进入目录问题详情；问题详情抽屉按解析失败、未索引、本地文件缺失、疑似变化、新增本地文件、索引不一致和 Embedding 不可用分组，并复用同步目录、重建索引、配置向量模型和复制路径操作
- 维护操作统一进入本地 FIFO 队列：导入目录、同步目录、补写索引、重建目录索引、重建全部索引、启用、停用和删除目录同一时间只运行一个任务
- 等待中的维护任务可以取消；运行中的任务不提供取消入口，避免 SQLite、Lucene 和图谱派生数据处于半更新状态
- 前端通过 `knowledge-maintenance` store 订阅 SSE 任务事件，任务完成后统一刷新目录列表、健康快照、已打开的目录详情和索引状态
- 导入、同步、重建、停用和删除等维护动作使用结构化二次确认弹窗；导入和重建类任务完成后展示需要用户确认的结果弹窗
...
`model_configs.embedding_requests_per_minute`、`embedding_tokens_per_minute` 和 `embedding_batch_size` 是 Embedding 专用限速字段。默认值分别是 `300`、`300000` 和 `16`，旧库启动时会幂等补列并为默认 Embedding 配置填充标准档位。
...
`app_settings` 是第 23 阶段新增的全局应用设置表，当前用于保存 `chat.query-contextualizer.mode`、`web-search.settings` 和 `ocr.settings`。聊天补全策略的优先级高于 `COGNINOTE_QUERY_CONTEXTUALIZER_MODE` 和旧的 `COGNINOTE_QUERY_CONTEXTUALIZER_ENABLED=false` 兼容开关；OCR 设置只保存开关和运行限制，VISION 模型密钥归属 `model_configs`。
...
`GET /api/ocr/settings`、`PUT /api/ocr/settings` 和 `POST /api/ocr/test` 用于 OCR 全局配置。第一版引擎固定为 `MODEL_VISION`；OCR 设置接口只返回当前视觉模型摘要和运行限制，不返回 API Key。测试接口使用后端生成的小测试图片验证当前 `VISION` 模型的图片输入能力，不上传用户文件，也不返回密钥。
...
- 资料总览页新增全库健康概览、目录健康 badge、问题数量、最近维护记录和问题抽屉；目录管理拆成独立列表面板，支持模糊搜索、状态筛选、分页查找和中文分页控件。
- 导入目录抽取为弹窗，停用目录作为 `DISABLED` 维护状态显示，不计入问题数量或全库健康告警。
- 问题弹窗先列出有问题的目录，问题抽屉按解析失败、未索引、本地文件缺失和疑似变化分组，并复用目录同步、目录重建和复制路径等显式修复动作。

### Milestone 32：知识库可信状态 P0 加固

- 健康总览新增 Lucene reader 实际文档数、chunk 数和 SQLite/Lucene 一致性判断。
- 新增 `INDEX_INCONSISTENT` 和 `EMBEDDING_UNCONFIGURED` 健康问题，分别提示重建索引和配置向量模型。
- 健康诊断覆盖本地目录新增文件提示，用户显式同步后才把新增文件写入 SQLite。
- 删除知识库目录时同步清理该目录 scope 下的维护记录，避免目录删除后继续显示孤儿可信状态数据。
- 可信状态控制台、目录管理列表和诊断抽屉统一展示 Lucene 一致性、Embedding 可用性、主要修复动作和维护记录入口。

### Milestone 33：知识库维护任务队列与统一状态模型

- `knowledge_folder_runs` 从完成后历史日志升级为维护任务队列和历史记录事实源，状态覆盖 `QUEUED`、`RUNNING`、`CANCELLING`、`CANCELLED`、`COMPLETED`、`COMPLETED_WITH_WARNINGS`、`FAILED`。
- 新增 `KnowledgeMaintenanceQueueService` 和 `/api/knowledge-maintenance/runs/**`，导入、同步、补写索引、重建、启停和删除目录统一入队并串行执行。
- 使用 SSE 推送维护任务快照、开始、进度、完成、失败和队列变化事件；页面刷新后通过 `GET /queue` 恢复当前任务和等待队列。
- 等待中的任务可以取消；运行中的任务不提供取消入口，避免用户误以为 SQLite、Lucene 或图谱派生数据可以随时安全回滚。
- 前端新增 `knowledge-maintenance` store，作为维护任务唯一状态源；任务终态后统一刷新队列、目录列表、健康快照、目录详情和索引状态。
- 导入、同步、补写索引、重建、停用和删除等维护动作补充结构化二次确认；导入、补写和重建类任务完成后进入用户确认式结果弹窗。
...
### Milestone 35：联网搜索 Tool Calling 与网页来源引用

- `ChatStreamRequest`、`AgentRequest` 和聊天设置弹层新增 `useWebSearch` 本轮开关。
- 新增 `WebSearchToolPolicy`，统一判断本轮开关、全局设置、provider 和 Exa API Key；不开启联网时不读取设置、不挂工具。
- 新增 `WebSearchTools.searchWeb`、`WebSearchClient`、`ExaWebSearchClient` 和请求级 `ToolExecutionCollector`，通过 Spring AI Tool Calling 让模型在需要时调用公开网页搜索。
- 新增 `/api/web-search/settings`、`PUT /api/web-search/settings` 和 `/api/web-search/test`，设置保存到 `app_settings`，响应只返回 `apiKeyConfigured`。
...
- 第一版只做 Lucene
- 第一版只做 `.md`、`.txt`、`.docx`、`.doc`、`.html`、`.htm` 和文本型 `.pdf`
- 第一版只做问答和溯源
- 其它都排到后续版本
...
- 保存文件路径、修改时间、内容哈希
- 资料总览页通过健康诊断提示目录不可访问、文件缺失、文件大小/mtime 疑似变化或目录新增文件
